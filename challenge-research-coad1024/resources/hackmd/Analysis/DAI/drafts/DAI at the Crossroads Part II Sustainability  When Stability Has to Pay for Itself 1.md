...
Backing and sustainability are not the same[1]. A vault can be perfectly overcollateralized on-chain yet economically unsustainable if market microstructure prevents collateral from being converted to DAI when needed. Black Thursday proved this distinction: the `Vat` invariants remained mathematically correct even as the protocol accumulated millions in bad debt[3].
...
Charged as interest accumulated into vault debt via the `rate` multiplier (Part I, Section 1.4)[1]. Current range: 0-8% annual depending on collateral type and market conditions[1].

**Purpose:** Discourage excessive debt minting, accumulate surplus for MKR buybacks, signal risk-adjusted cost of capital.
...
2. **Network throughput** (finite transaction capacity)
3. **Market liquidity** (finite depth at forced liquidation volumes)

During crashes, all three constraints bind simultaneously. Makers response was pragmatic: lower fees to encourage participation, introduce PSM for instant USDC arbitrage, extend auction durations from 10 min → 6 hours to give more time for price discovery[1][3].

**3. Dai Savings Rate (DSR) — Interest Paid to DAI Holders**

Introduced post-Black Thursday to make holding DAI attractive during peg stress. Currently 0-8%, deliberately set below stability fees to maintain deficit neutrality[1][3].

**Purpose:** When DAI trades above $1, raising DSR incentivizes selling DAI onto the market (increasing supply, driving price down).

**Limitation:** DSR doesn't solve the fundamental problem—it just moves value around. High DSR means vault owners pay more in fees to compensate DAI holders. Under extreme stress, neither fee adjustment nor DSR can restore peg if underlying collateral fails to provide anchor[2][3][4].
...
4. Reduced stablecoin demand (fewer new vaults) + panic selling = DAI premium (trades above $1)
5. DAI premium paradoxically *worsens* the spiral: it's a "short squeeze" on vault owners trying to repurchase DAI to close positions
6. Higher DAI price makes collateral effectively worth less in stablecoin terms → triggers more liquidations

This is counterintuitive but mathematically inevitable: **DAI appreciates precisely when collateral crashes, tightening the squeeze on everyone trying to deleverage**[2][4].
...
### 2.4 Loop 3: Governance — Dynamic Hedging and the MKR Backstop

Governance serves as the dynamic hedge of last resort[1][3][4]. When collateral fails (Loop 1) and incentive mechanisms can't recover value (Loop 2), governance deploys MKR as equity to absorb losses.
...
**Flap/Flop Auctions: MKR as Equity**

When system debt (sin) exceeds surplus (joy) after 6.5-day debt queue:

- **Flop auction:** Mint and sell MKR to raise DAI, covering shortfall. MKR holders absorb loss through dilution[1][3]
- **Flap auction:** Burn MKR with excess DAI, returning value to holders during surpluses[1][3]

This is the ultimate backstop: MKR is literally Maker's equity. During Black Thursday, ~4-6M in debt accumulated, requiring ~500k MKR to be minted and sold (~$28M at then-prices, representing 3%+ dilution in holders' equity[3]).
...
3. **Voter coordination:** MKR holders must coordinate under stress; incentives misaligned (small holders may exit rather than participate)[3][4]
...
The system works precisely because it stays tightly balanced, but this balance is fragile.
...
Maker's liquidation pipeline processes auctions sequentially[1][3]. During March 12-13, 2020:
...
**The cascade effect:** When liquidation demand exceeds capacity, auctions fall behind market price. Collateral devalues faster than it can be sold. Each auction round intensifies the next[2][3][4].
...
**Maker's response:** `Dog.hole` parameter limits active liquidation debt system-wide (~100M DAI as of 2024). This prevents unlimited auction creation but creates a new problem: once hole is reached, new liquidations queue but don't execute. Vault owners aren't liquidated for hours, meaning their true collateral value isn't discovered until much later[1][3].

**2. Oracle Delay: The One-Hour Information Asymmetry**

The `OSM` maintains a 1-hour delay to give vault owners time to adjust positions[1]. This works well during price drift. During crashes, it becomes deadly[3].
...
|--------|-----------|--------------|
| **Current (1h fixed delay)** | User reaction time | Lag creates cascades |
...
- Should I deploy capital to Maker auctions or chase yield elsewhere?
- During crashes, Maker auctions become riskier (more volatile collateral)
...
**The cascade:** When keeper participation drops below critical threshold, auctions clear at 50-70% of market value instead of 95%+. This creates realized losses for vault owners and system deficits. The deficit then forces governance to dilute MKR, which reduces MKR holders' interest in supporting the system, potentially cascading to reduced governance participation[3][4].

---

## Part III: Black Thursday — Empirical Validation of Theory

The March 12-13, 2020 crisis transformed theoretical fragility into observed fact[3][4]. It remains the most comprehensive real-world test of stablecoin crisis dynamics.

### 2.6 Crisis Sequence and Quantitative Outcomes

**Macro trigger:**

COVID-19 pandemic panic. S&P 500 fell ~12% on March 9-12. Equities, commodities, and crypto all sold off simultaneously—a systemic risk event where all correlations converge to 1[3][4].

**Timeline (hours):**

| Time (UTC) | Event | Market Impact |
|-----------|-------|----------------|
| **Mar 12, 08:00** | Crypto selling begins | ETH ~$195 |
| **Mar 12, 12:00** | Flash crash across markets | ETH $195 → $110 (43% drop) in minutes |
| **Mar 12, 12:00-13:00** | Oracle delay period | OSM still reporting ~$195; vaults appear safe |
| **Mar 12, 13:00** | OSM updates; cascade begins | 4,600 liquidations triggered in 18 hours |
| **Mar 12-13** | Auction failures; zero-bid phenomenon | ~70% of auctions fail to cover debt |
| **Mar 15-17** | Community emergency response | Fee cuts, USDC-A introduction, parameters adjusted |

**Quantitative outcomes (Kjaer 2021):[3]**

| Metric | Normal Period | Black Thursday | Recovery Period |
|--------|--------------|-----------------|-----------------|
| **Auctions reaching dent phase** | 97% | 29.6% | 100% (by March 17) |
| **Auction effectiveness (median)** | 97.4% | 76.9% | 97%+ |
| **Liquidation delay (median)** | 0.2 min | 2.6 min | 0.2 min |
| **DAI price** | $0.99-$1.01 | $1.11 peak | $1.00-$1.01 |
| **System bad debt (sin)** | Near-zero | $4-6M | Recovered via MKR dilution |
| **MKR dilution needed** | ~0% | ~3% of supply | Permanent dilution |

**Zero-bid phenomenon: The mempool attack[3]**

Approximately 10-15% of Black Thursday liquidations cleared with bids of ~$0 per ETH (literally pennies). This wasn't random—forensic analysis revealed:

1. Attackers flooded mempool with low-gas-price transactions
2. Auction keeper bots, blinded by distorted gas price oracles, sent transactions with insufficient gas
3. These transactions never mined; nonce gaps prevented later retries
4. Attackers' high-gas transactions got included, allowing them to buy ETH at zero prices
5. Attacker profit: ~$8M recovered from effectively stealing collateral[3]

This wasn't a smart contract bug—it was a layer-1 (mempool) attack enabled by Maker's tight auction parameters (10-minute duration meant no time for retry) combined with network congestion[3].

**Recovery dynamics post-March 13:**

Governance response was impressively fast (72 hours for major parameter changes). By March 17, USDC-A introduction created instant arbitrage channel for peg recovery. PSM enabled:
- Arbitrageurs to mint DAI by depositing USDC (since PSM charged near-zero fees)
- DAI supply increased, price fell back to peg
- System bad debt was eventually recovered through Flop auctions (MKR dilution)[3]

By May 2020, the system had fully absorbed the Black Thursday loss. But the cost was permanent: ~500k MKR minted and sold, representing ~3% dilution to existing holders[3].
...
**Interpretation:** In stable domain, stablecoin prices cluster tightly around $1, variance is bounded, large deviations are rare. This matches observed Maker behavior for 99% of history[3][4].
...
7. Feedback loop: more liquidations → higher DAI price → more liquidations[2][3][4]
...
Or equivalently, liquidation ratio requirements of 150-200% (which Maker uses)[2][4].
...
All four occurred on March 12: ETH was negative drift (crypto correlation event), correlated with all leverage unwinding (everyone liquidating simultaneously), PSM didn't exist yet (no alternate supply), and keepers were overwhelmed[3][4].
...
1. **Stability fees:** Vault borrowers pay interest (0-8% annual depending on collateral type)[1]
...
Surplus accumulates from fees → exceeds "hump" threshold (~2M DAI) → triggers Flapper auction → MKR burned using excess DAI → MKR supply falls → remaining holders' equity increases[1][3].

**Example (2020-2021 normal period):**
- Monthly stability fee income: ~$5-10M
- Monthly liquidation penalties: ~$1-2M
- Monthly DSR cost: ~$2-5M
- Monthly net: +$2-5M to surplus

Over 12 months, this accumulates to ~$30-60M, funding occasional MKR buybacks and offsetting small deficits[3].
...
During normal markets, this is achievable. Fee rates are set such that vault owners pay enough to cover both compensation to DAI savers (DSR) and system reserves[1][3].
...
| **Total system deficit** | ~$4-6M DAI | Accumulates in Vow.sin |
...
To cover ~$4.6M deficit with 0.1% MKR dilution, system needs:

\[
\text{MKR minted} \approx \frac{\text{Deficit}}{MKR\ price}
\]

At ~$11 USD/MKR price (March 2020), this meant ~420k MKR to mint and sell. At then-market cap of ~$40B, this represented ~1% dilution to existing holders (effectively paying them for absorbing the loss)[3][4].
...
- **MKR holders** absorb unlimited tail risk through dilution

This is the intended capital structure—MKR is equity, absorbs losses—but it creates a moral hazard: MKR holders may exit if risks become unbounded[3][4].

---

### 2.12 PSM Impact: Trading Sustainability for Short-term Stability

The PSM fundamentally altered economics by introducing instant USDC↔DAI swaps at 1:1 ratio[1][3].

**Economic trade-offs:**

| Aspect | Pre-PSM | Post-PSM |
|--------|---------|----------|
| **Peg stability** | Volatile (±5% swings) | Tight (±0.5%) |
| **Revenue model** | Fees from 100% vaults | Fees from ~60-70% of DAI |
...
If 40% of DAI is now PSM-backed (up from 0%):
- Lost annual revenue: 40% × 3% (midpoint) = 1.2% of DAI supply in fees
- ~$50M annually (at $4B DAI supply) that can't be captured

This is the explicit trade-off: short-term peg stability (PSM helps instantly) costs long-term sustainability (reduced surplus generation)[1][3].
...
The Sustainability Triangle is not a fixed structure—it's a dynamic balance that requires constant retuning[1][3][4]. Black Thursday proved that static systems fail under stress; DAI survives because Maker's community continuously adjusts the triangle's three axes[3][4].

**Key insights:**

**1. Backing ≠ Sustainability**
On-chain invariants (Part I) guarantee arithmetical backing, not economic sustainability. DAI can be mathematically backed yet economically unsustainable if market microstructure fails[2][3][4].
...
**5. MKR holders are the equity**
In tail events, MKR absorbs losses through dilution. This is intentional, but it creates a finite tolerance for repeated shocks. Too many Black Thursdays and MKR holders may exit entirely[3][4].
...
- Making MKR a "central reserve currency" rather than individual protocol token

This trades decentralization for scalability and resilience. Whether it succeeds depends on execution and market adoption.

**For Part III** (Governance, to be written): The question becomes—when MKR becomes system-critical infrastructure, how does governance remain credibly neutral and resistant to capture while maintaining the dynamic responsiveness needed in crisis?
...
[2] Klages-Mundt, A. (2023). *Stablecoin Risk Models*. PhD Dissertation, Cornell University. arXiv:1303.6969 (Thesis chapters on deleveraging spirals and regime transitions)

[3] Kjaer, M. (2021). *Quantitative Analysis of MakerDAO's Liquidation System*. Diploma Thesis, TU Wien. Chapters 4-5 contain detailed Black Thursday analysis including metrics M1-M9.

[4] Klages-Mundt, A., & Minca, A. (2022). "While Stability Lasts: A Stochastic Model of Noncustodial Stablecoins." Management Science (published); arXiv preprint version contains extended appendix with formal proofs.
