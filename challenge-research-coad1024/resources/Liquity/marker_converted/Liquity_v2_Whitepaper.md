...
1
DISCLAIMER: The outlined ideas for the Liquity Protocol V2 are for informational purposes only. The
...
of collateral assets with the terms being subject to human governance. After an initial period
of peg issues, Aave introduced a yield source for GHO depositors, eventually stabilizing its
...
only thrive if there’s sufficient demand for holding and transacting with it. In times of positive
...
on slow and potentially misaligned human governance to adjust interest rates (DAI, GHO), or
...
of their loans, benefiting from greater flexibility in time.
...
opposite spreads: depending on the utilization and integration of the stablecoin in broader
DeFi, the Stability Pool yields may even exceed the average interest rates, something that


2
See below “Interest rate adjustment and premature adjustment fee” regarding the fee charged on
overly frequent interest rate adjustments.

3
For the given collateral. See below.
...
4
Disregarding the yield earned on the collateral, which in case of LSTs is close to 0%.
...
5
In Liquity V1, the borrowing costs are fixed, whereas the LQTY emissions for holding LUSD in the
...
overcollateralized. Holders are thus subject to the risks of all supported collateral
...
6
In Liquity V1 the Recovery Mode is mainly needed due to a lack of sustainable yield for the Stability
...
7
In a worst case scenario where a collateral asset suddenly breaks down or loses most of its value,
...
8
Borrowing markets with less risky or higher quality collateral assets are likely to have lower interest
...
9
If the user changes their stake later on (e.g. by staking more LQTY or unstaking a part of it), the
...
10
The collateral assets are hardcoded and not subject to governance.



11
The provided ETH gets seamlessly wrapped into WETH and unwrapped upon withdrawal.



12
The system further requires a minimum debt of 2000 BOLD amount to open a position, making sure
...
13
The opening of the position and the delegation of rate management to a batch manager are treated
the same way as an interest rate adjustment.



14
Example: if the average interest rate of a collateral branch is 5%, the borrower has to pay a fee of
0.096% on each adjustment.



15
Savvy users could anticipate upcoming redemptions (including via watching the mempool) and
...
16
Liquidation is a permissionless function that can be called by anybody.
...
17
The protocol treats 1 BOLD as equal to $1. However, the market value of BOLD may be higher than
$1 especially in case of mass liquidations and liquidity crunches, reducing the liquidation gains.

18
The two operations can be combined in a single transaction.
...
19
See the chart at https://dune.com/queries/81466/162640.

20
The waiting time protects the delegating borrowers from a rapid debt increase due to overly
...
or removing collateral or by borrowing or repaying BOLD. This allows delegates to offer
...
21
Both the distributed collateral and the burned debt are determined pro rata to the size of the BOLD
deposits.
...
22
Just as in Liquity V1, this restriction makes it harder for depositors to evade liquidations.

23
Depositors will also have the option between paying out the pending gains when adjusting their
...
amount of collateral. The actual amount **c** of collateral that is taken out from the affected
...
24
As well as avoiding a number of potential edge cases and attack surfaces where the system is
pulled into a state with TCR < CT.



25
Either the call to the oracle reverts, or returns 0, or returns a price that is too stale.



26
Besides that, regular redemptions and liquidations remain possible.



27
Note the incentive might disappear when BOLD is trading significantly above peg.
